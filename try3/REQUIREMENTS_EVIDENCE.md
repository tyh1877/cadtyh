...
| V2 uses MEP, interface graph, feature graph and basic CAD operation skills | schemas + validators + skill logs | per-case generated plans and Fusion feature logs | partial — basic-operation jobs prepared; Fusion rerun required |
| Fusion is real and editable | `results/fusion_api_skill_smoke.md` and basic-operation smoke script | generic operation skills create real features; rebuild + F3D/STEP/STL pass | pending — basic-operation smoke must be rerun in Fusion |
...
| Gap metrics are deterministic | `evaluate_fusion_api.py`, `evaluate_interface_consistency.py` | global/per-link/joint-local/interface tables | pending — rerun after basic-operation Fusion batch |
