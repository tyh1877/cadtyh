...
| CAD–Kinematic Consistency | `results/fusion_api_interface_consistency.csv` | link-component correspondence and canonical placement pass; interface gaps/overlap remain poor | partial — insufficient for a positive method claim |
...
| V2 uses MEP, interface graph, feature graph and explicit basic CAD operation skills | schemas + validators + skill logs | per-case generated plans and Fusion feature logs | pass for 4/5 V2 generated cases; 1 upstream blueprint failure retained |
| Fusion is real and editable | `results/fusion_api_skill_smoke.md` and basic-operation smoke script | sketch/extrude/loft and modifier operations create real features; rebuild + F3D/STEP/STL pass | pass — smoke 10/10 calls; formal batch 8/10 jobs with two upstream generation failures |
...
| Gap metrics are deterministic | `evaluate_fusion_api.py`, `evaluate_interface_consistency.py` | global/per-link/joint-local/interface tables | pass — 8 successful Fusion jobs evaluated, two upstream failures retained |
...
| Resource accounting and report exist | manifests, execution logs, `try3_report.md` | token/call/Fusion-op/latency tables + report | partial — report updated; visual contact sheets remain absent |
