...
try:
 for link in s['links']:
  lid=link['link_id'];source_path=root/f'experiments/try5A/A0/{lid}';source=App.openDocument(str(source_path/'model.FCStd'));ir=json.loads((source_path/'cad_ir.json').read_text());shape=Part.makeCompound([source.getObject(name).Shape.copy() for name in ir['final_objects']]);App.closeDocument(source.Name);obj=doc.addObject('Part::Feature','Link_'+lid);obj.Label='Link_'+lid;obj.Shape=shape;m=App.Matrix();T=link['world_transform_canonical'];
  # FreeCAD matrix translation is millimetres.
  for r in range(3):
   for c in range(3):setattr(m,f'A{r+1}{c+1}',T[r][c])
  m.A14=T[0][3]*1000;m.A24=T[1][3]*1000;m.A34=T[2][3]*1000;m.A41=0;m.A42=0;m.A43=0;m.A44=1;obj.Placement=App.Placement(m);obj.addProperty('App::PropertyString','SourceLink').SourceLink=lid;objects.append(obj)
