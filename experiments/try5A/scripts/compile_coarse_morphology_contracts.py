...
import numpy as np
ROOT=Path(__file__).resolve().parents[3];HERE=ROOT/'experiments/try5A';POLICY=ROOT/'try5/knowledge/coarse_morphology/gate_policy.json';PLAN=HERE/'blackboard/robot_assembly_plan.json';SKEL=HERE/'blackboard/kinematic_skeleton.json';OUT=HERE/'blackboard/coarse_morphology_contracts.json'
policy=json.loads(POLICY.read_text());plan=json.loads(PLAN.read_text());skel=json.loads(SKEL.read_text());plans={x['link_id']:x for x in plan['links']};joints=skel['joints'];contracts=[]
...
out={'schema_version':'try5A_coarse_morphology_contract_v1','compiled_before_cad_evaluation':True,'sources':{'policy':str(POLICY.relative_to(ROOT)).replace('\\','/'),'policy_sha256':hashlib.sha256(POLICY.read_bytes()).hexdigest(),'plan_sha256':hashlib.sha256(PLAN.read_bytes()).hexdigest(),'skeleton_sha256':hashlib.sha256(SKEL.read_bytes()).hexdigest()},'thresholds':policy['thresholds'],'links':contracts,'functional_subassemblies':groups,'visual_evidence_required':['front','side','top','isometric']};OUT.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps({'links':len(contracts),'subassemblies':len(groups),'output':str(OUT.relative_to(ROOT))},indent=2))
