...
def prepare_scaffold_ablation(config_path):
    config_path=Path(config_path).resolve(); config=load(config_path); result_root=HERE/"results/try5b1_a1_frozen_scaffold"; result_root.mkdir(parents=True,exist_ok=True)
    lock_path=ROOT/config["shared"]["formal_holdout_lock"]
    if lock_path.exists(): raise FileExistsError("formal holdout lock already exists")
    parity=audit_condition_parity(config)
    if parity["status"]!="PASS" or parity["observed_differences"]!=["mechanical_geometry_policy.preserve_frozen_scaffold"]: raise RuntimeError("single-variable parity failed")
    split=load(ROOT/config["shared"]["development_split"]); development_input=load(ROOT/config["shared"]["development_input"]); development_ids={item["config_id"] for item in development_input["coupled_configurations"]}; holdout_ids=set(split["holdout"]["case_ids"])
    if len(development_ids)!=96 or development_ids!=set(split["development"]["case_ids"]) or development_ids & holdout_ids or development_input["contains_formal_holdout_configurations"]: raise RuntimeError("development-only input validation failed")
    commit=subprocess.run(["git","rev-parse","HEAD"],cwd=ROOT,capture_output=True,text=True,check=True).stdout.strip(); protocol_hash=sha(config_path)
    lock={"schema_version":"robotcad_formal_holdout_lock_v1","experiment_id":config["experiment_id"],"case_ids":split["holdout"]["case_ids"],"case_ids_sha256":split["holdout"]["case_ids_sha256"],"case_count":split["holdout"]["count"],"protocol_path":_repo_relative(config_path),"protocol_sha256":protocol_hash,"git_commit":commit,"creation_timestamp_utc":_utc_now(),"accessed":False,"evaluation_count":0,"generator_access":False,"ordinary_runner_access":False}; dump(lock_path,lock)
    ...
    specs=load(ROOT/config["shared"]["body_family_specs"]); f2_specs={link_id:specs[link_id]["F2"] for link_id in config["shared"]["pilot_links"]}; images={view:{"path":config["shared"]["formal_image_root"]+"/"+view+".png","sha256":sha(ROOT/config["shared"]["formal_image_root"]/(view+".png"))} for view in config["shared"]["formal_image_views"]}
    pre_run={"schema_version":"robotcad_scaffold_ablation_pre_run_v1","status":"READY","implementation_commit":commit,"protocol_sha256":protocol_hash,"development_input":{"path":config["shared"]["development_input"],"sha256":sha(ROOT/config["shared"]["development_input"]),"configuration_count":len(development_ids),"case_id_set_sha256":development_input["case_id_set_sha256"]},"holdout_lock":{"path":config["shared"]["formal_holdout_lock"],"sha256":sha(lock_path),"accessed":False},"frozen_inputs":{"images":images,"sanitized_urdf":{"path":config["shared"]["sanitized_urdf"],"sha256":sha(ROOT/config["shared"]["sanitized_urdf"])},"f2_schema":{"sha256":canonical_hash(f2_specs),"links":f2_specs},"visual_evidence_packs":{link_id:{"path":config["shared"]["visual_evidence_root"]+"/"+link_id+".json","sha256":sha(ROOT/config["shared"]["visual_evidence_root"]/(link_id+".json"))} for link_id in config["shared"]["pilot_links"]},"interface_contract":{"path":config["shared"]["interface_contracts"],"sha256":sha(ROOT/config["shared"]["interface_contracts"])},"semantic_inventories":{link_id:sha(ROOT/config["shared"]["semantic_inventory_root"]/(link_id+".json")) for link_id in config["shared"]["pilot_links"]},"mechanical_topology":{link_id:sha(ROOT/config["shared"]["mechanical_topology_root"]/(link_id+".json")) for link_id in config["shared"]["pilot_links"]},"seed":config["shared"]["seed"],"joint_sweeps":config["shared"]["relevant_joint_sweeps"],"joint_sweep_fractions":config["shared"]["joint_sweep_fractions"]},"evaluator_settings":{"exact":{"path":"experiments/try5A/evaluation/mechanical/freecad_holdout_evaluator.py","sha256":sha(HERE/"evaluation/mechanical/freecad_holdout_evaluator.py")},"geometry":{"path":"experiments/try5A/evaluation/geometry_pair_evaluator.py","sha256":sha(HERE/"evaluation/geometry_pair_evaluator.py"),"sample_count":config["shared"]["geometry_sample_count"]},"threshold_tuning":False},"runner_sha256":sha(Path(__file__)),"worker_sha256":sha(HERE/"evaluation/mechanical/freecad_link_refinement.py")}; dump(result_root/"pre_run_manifest.json",pre_run)
    print(json.dumps({"status":"READY","lock":_repo_relative(lock_path),"holdout_cases":lock["case_count"],"accessed":False,"parity":parity["status"]},indent=2)); return 0


def run_scaffold_ablation_development(config_path):
    config_path=Path(config_path).resolve(); config=load(config_path); result_root=HERE/"results/try5b1_a1_frozen_scaffold"; artifact_root=HERE/"artifacts/try5b1_a1_frozen_scaffold"; artifact_root.mkdir(parents=True,exist_ok=True)
    lock_path=ROOT/config["shared"]["formal_holdout_lock"]; lock=load(lock_path); pre_run=load(result_root/"pre_run_manifest.json")
    if lock["accessed"] or lock["evaluation_count"]!=0 or lock["protocol_sha256"]!=sha(config_path): raise RuntimeError("formal holdout lock invalid or consumed")
    started_path=result_root/"formal_development_started.json"
    if started_path.exists(): raise FileExistsError("formal development already attempted; automatic rerun forbidden")
    development=load(ROOT/config["shared"]["development_input"]); coupled=development["coupled_configurations"]; per_joint=development["per_joint"]; canonical=development["canonical_config"]
    split=load(result_root/"case_split.json"); development_ids={item["config_id"] for item in coupled}; holdout_ids=set(split["holdout"]["case_ids"])
    if len(coupled)!=96 or development_ids & holdout_ids or development_ids!=set(split["development"]["case_ids"]): raise RuntimeError("ordinary runner refused non-development input")
    dump(started_path,{"status":"STARTED_NO_RETRY","timestamp_utc":_utc_now(),"git_commit":subprocess.run(["git","rev-parse","HEAD"],cwd=ROOT,capture_output=True,text=True,check=True).stdout.strip(),"protocol_sha256":sha(config_path),"development_input_sha256":sha(ROOT/config["shared"]["development_input"]),"formal_holdout_accessed":False})
    specs=load(ROOT/config["shared"]["body_family_specs"]); f2_specs={link_id:specs[link_id]["F2"] for link_id in config["shared"]["pilot_links"]}; runs={}; failure_rows=[]
    for link_id in config["shared"]["pilot_links"]:
        for condition,condition_config in config["conditions"].items():
            run_id=link_id+"__"+condition; run_root=artifact_root/"formal_runs"/link_id/condition; output=run_root/"generation_result.json"; job_path=run_root/"generation_job.json"
            if output.exists() or job_path.exists(): raise FileExistsError("run artifact already exists: "+run_id)
            relevant=config["shared"]["relevant_joint_sweeps"][link_id]; job={"experiment_id":config["experiment_id"],"run_id":run_id,"phase":"FORMAL_DEVELOPMENT","policy_condition":condition,"mechanical_geometry_policy":condition_config["mechanical_geometry_policy"],"conditions":["F2"],"pilots":{link_id:{"F2":f2_specs[link_id]}},"coupled":coupled,"per_joint":{joint_id:per_joint[joint_id] for joint_id in relevant},"relevant_joints":relevant,"canonical_config":canonical,"cad_root":str(run_root/"cad"),"assembly_root":str(run_root/"assemblies"),"output":str(output),"evaluation_enabled":False,"reuse_exact":False,"gt_access":False,"repair_enabled":False,"retry_allowed":False,"formal_holdout_access":False}
            dump(job_path,job); process=subprocess.run([python_runtime(),str(HERE/"evaluation/mechanical/freecad_link_refinement.py"),str(job_path)],cwd=ROOT,capture_output=True,text=True,timeout=1800); (run_root/"stdout.txt").write_text(process.stdout,encoding="utf-8"); (run_root/"stderr.txt").write_text(process.stderr,encoding="utf-8")
            if process.returncode:
                failure_rows.append({"run_id":run_id,"link_id":link_id,"condition":condition,"stage":"generation","requested_cases":96,"completed_cases":0,"failed_cases":96,"returncode":process.returncode,"error":(process.stderr or process.stdout)[-2000:]}); continue
            worker=load(output); build=next(item for item in worker["builds"] if item["condition"]=="F2" and item["link_id"]==link_id); runs[run_id]={"link_id":link_id,"condition":condition,"worker":worker,"build":build,"run_root":run_root,"job_path":job_path}
    dump(result_root/"generation_failure_accounting.json",{"requested_runs":6,"completed_runs":len(runs),"failed_runs":6-len(runs),"failures":failure_rows,"retry_count":0})
    if failure_rows: raise RuntimeError("formal generation failure retained; experiment stopped without retry")

    mechanical_by={}; evaluator_artifacts={}
    for link_id in config["shared"]["pilot_links"]:
        pair_root=artifact_root/"exact_evaluation"/link_id; candidate={"schema_version":"robotcad_scaffold_pair_candidates_v1","frozen_nonpilot_root":config["shared"]["frozen_nonpilot_root"],"interface_contracts":config["shared"]["interface_contracts"],"conditions":{}}
        for condition in config["conditions"]:
            build=runs[link_id+"__"+condition]["build"]; candidate["conditions"][condition]={"selected_candidate":"F2","pilot_fcstd":{link_id:_repo_relative(build["artifact"]["fcstd"])},"mechanical_geometry_policy":config["conditions"][condition]["mechanical_geometry_policy"]}
        candidate_path=pair_root/"candidate_manifest.json"; dump(candidate_path,candidate); relevant=config["shared"]["relevant_joint_sweeps"][link_id]; output=pair_root/"mechanical_raw.json"; job={"mode":"SCAFFOLD_ABLATION_DEVELOPMENT_EXACT","candidate_manifest":str(candidate_path),"configurations":coupled,"per_joint":{joint_id:per_joint[joint_id] for joint_id in relevant},"output":str(output)}; job_path=pair_root/"evaluator_job.json"; dump(job_path,job); process=subprocess.run([python_runtime(),str(HERE/"evaluation/mechanical/freecad_holdout_evaluator.py"),str(job_path)],cwd=ROOT,capture_output=True,text=True,timeout=1800); (pair_root/"stdout.txt").write_text(process.stdout,encoding="utf-8"); (pair_root/"stderr.txt").write_text(process.stderr,encoding="utf-8")
        if process.returncode: raise RuntimeError("Exact evaluator failed for "+link_id+": "+(process.stderr or process.stdout))
        mechanical_by[link_id]={item["condition"]:item for item in load(output)["conditions"]}; evaluator_artifacts[link_id]={"path":_repo_relative(output),"sha256":sha(output)}

    geometry_job={"gt_urdf":"go_nogo1/sources/urdf_files_dataset/urdf_files/robotics-toolbox/xacro_generated/interbotix_descriptions/urdf/px100.urdf","seed":config["shared"]["seed"],"candidates":[],"output":str(result_root/"geometry_raw.json")}
    for link_id in config["shared"]["pilot_links"]:
        for condition in config["conditions"]:
            build=runs[link_id+"__"+condition]["build"]; geometry_job["candidates"].append({"condition":condition,"link_id":link_id,"body_stl":_repo_relative(build["artifact"]["body_stl"]),"final_stl":_repo_relative(build["artifact"]["stl"])})
    geometry_job_path=artifact_root/"geometry_evaluator_job.json"; dump(geometry_job_path,geometry_job); geometry_process=subprocess.run([sys.executable,str(HERE/"evaluation/geometry_pair_evaluator.py"),str(geometry_job_path)],cwd=ROOT,capture_output=True,text=True,timeout=1800); (artifact_root/"geometry_stdout.txt").write_text(geometry_process.stdout,encoding="utf-8"); (artifact_root/"geometry_stderr.txt").write_text(geometry_process.stderr,encoding="utf-8")
    if geometry_process.returncode: raise RuntimeError(geometry_process.stderr or geometry_process.stdout)
    geometry=load(geometry_job["output"]); geometry_by={(item["link_id"],item["condition"],item["scope"]):item for item in geometry["rows"]}

    table=[]; traces=[]; evaluated_failures=[]
    for link_id in config["shared"]["pilot_links"]:
        for condition in config["conditions"]:
            build=runs[link_id+"__"+condition]["build"]; exact=mechanical_by[link_id][condition]; final_geometry=geometry_by[(link_id,condition,"final_assembled_link")]; body_geometry=geometry_by[(link_id,condition,"refined_body_only")]; jr3_values=[item["jr3"] for item in exact["per_joint"]]; row={"link":link_id,"condition":condition,"bicr":1.0 if build["attachment_valid"] else 0.0,"attachment_valid":build["attachment_valid"],"connected_solid_count":build["solid_count"],"jr3":min(jr3_values) if jr3_values else None,"jr3_detail":exact["per_joint"],"gcfr":exact["gcfr"],"collision_events":exact["collision_events"],"intersection_volume_mm3":exact["intersection_volume_mm3"],"failed_configuration_count":exact["invalid_configurations"],"failed_configuration_ids":exact["failed_configuration_ids"],"final_voxel_iou":final_geometry["voxel_iou"],"final_silhouette_iou":final_geometry["silhouette_iou_mean"],"final_normalized_chamfer":final_geometry["normalized_chamfer"],"final_normalized_hd95":final_geometry["normalized_hd95"],"final_bbox_error_mm":final_geometry["bbox_error_mm"],"final_major_dimension_error_mm":final_geometry["major_dimension_error_mm"],"body_only_diagnostic":body_geometry}; table.append(row); evaluated_failures.append({"run_id":link_id+"__"+condition,"requested_cases":96,"completed_cases":exact["configuration_count"],"failed_evaluation_cases":exact["invalid_configurations"],"failed_configuration_ids":exact["failed_configuration_ids"]})
        s1=runs[link_id+"__S1_SCAFFOLD_PRESERVED"]["build"]; s0=runs[link_id+"__S0_SCAFFOLD_REMOVED"]["build"]; traces.append({"link_id":link_id,"raw_body_signature_equal":s1["raw_body_signature"]==s0["raw_body_signature"],"body_only_stl_sha256":{"S1":sha(s1["artifact"]["body_stl"]),"S0":sha(s0["artifact"]["body_stl"])},"body_only_equal":sha(s1["artifact"]["body_stl"])==sha(s0["artifact"]["body_stl"]),"final_stl_sha256":{"S1":sha(s1["artifact"]["stl"]),"S0":sha(s0["artifact"]["stl"])},"final_equal":sha(s1["artifact"]["stl"])==sha(s0["artifact"]["stl"]),"final_volume_mm3":{"S1":s1["artifact"]["volume_mm3"],"S0":s0["artifact"]["volume_mm3"]},"scaffold_volume_delta_mm3":s1["artifact"]["volume_mm3"]-s0["artifact"]["volume_mm3"],"S1_execution_trace":s1["execution_trace"],"S0_execution_trace":s0["execution_trace"],"only_effective_difference":"frozen_scaffold_preservation_fusion"})
    dump(result_root/"paired_main_table.json",{"rows":table}); csv_fields=["link","condition","bicr","connected_solid_count","jr3","gcfr","collision_events","intersection_volume_mm3","final_voxel_iou","final_silhouette_iou","final_normalized_chamfer","final_normalized_hd95","final_bbox_error_mm","final_major_dimension_error_mm"]; rows_csv(result_root/"paired_main_table.csv",[{field:row[field] for field in csv_fields} for row in table]); dump(result_root/"protection_trace.json",{"links":traces})
    metrics=["bicr","connected_solid_count","jr3","gcfr","collision_events","intersection_volume_mm3","final_voxel_iou","final_silhouette_iou","final_normalized_chamfer","final_normalized_hd95","final_bbox_error_mm","final_major_dimension_error_mm"]
    by={(row["link"],row["condition"]):row for row in table}; deltas=[]
    for link_id in config["shared"]["pilot_links"]:
        delta={"link":link_id,"direction":"S1_minus_S0"}
        for metric_name in metrics:
            a=by[(link_id,"S1_SCAFFOLD_PRESERVED")][metric_name]; b=by[(link_id,"S0_SCAFFOLD_REMOVED")][metric_name]; delta[metric_name]=None if a is None or b is None else a-b
        deltas.append(delta)
    aggregates={}
    for condition in config["conditions"]:
        condition_rows=[row for row in table if row["condition"]==condition]; aggregates[condition]={}
        for metric_name in metrics:
            values=[row[metric_name] for row in condition_rows if row[metric_name] is not None]; aggregates[condition][metric_name]={"mean":mean(values),"median":median(values)} if values else {"mean":None,"median":None}
    dump(result_root/"paired_deltas.json",{"per_link":deltas,"aggregate_by_condition":aggregates})
    failure_accounting={"generation":{"requested_runs":6,"completed_runs":6,"failed_runs":0,"retry_count":0},"evaluation":{"rows":evaluated_failures,"requested_cases_per_run":96,"denominator_preserved":all(item["completed_cases"]==item["requested_cases"] for item in evaluated_failures)},"formal_holdout":{"accessed":False,"evaluation_count":0}}; dump(result_root/"failure_accounting.json",failure_accounting)
    dump(result_root/"claim_ledger.json",{"claims":[{"claim_id":"six_runs_complete","hard":True,"evidence_type":"computed","artifact":"failure_accounting.json","field":"generation.completed_runs","operator":"eq","expected":6},{"claim_id":"denominator_preserved","hard":True,"evidence_type":"computed","artifact":"failure_accounting.json","field":"evaluation.denominator_preserved","operator":"eq","expected":True},{"claim_id":"holdout_unaccessed","hard":True,"evidence_type":"computed","artifact":"failure_accounting.json","field":"formal_holdout.accessed","operator":"eq","expected":False}]})
    manifest={"experiment_id":config["experiment_id"],"phase":"FORMAL_DEVELOPMENT_COMPLETE_HOLDOUT_UNACCESSED","config_path":_repo_relative(config_path),"config_file_sha256":sha(config_path),"config_canonical_sha256":canonical_hash(config),"case_split_sha256":split["split_sha256"],"formal_holdout_lock_sha256":sha(lock_path),"pre_run_manifest_sha256":sha(result_root/"pre_run_manifest.json"),"runner_sha256":sha(Path(__file__)),"worker_sha256":sha(HERE/"evaluation/mechanical/freecad_link_refinement.py"),"exact_evaluator_sha256":sha(HERE/"evaluation/mechanical/freecad_holdout_evaluator.py"),"geometry_evaluator_sha256":sha(HERE/"evaluation/geometry_pair_evaluator.py"),"development_input_sha256":sha(ROOT/config["shared"]["development_input"]),"exact_raw_artifacts":evaluator_artifacts,"geometry_raw_sha256":sha(result_root/"geometry_raw.json"),"formal_holdout_accessed":False,"formal_holdout_evaluation_count":0}; dump(result_root/"manifest.json",manifest)
    print(json.dumps({"status":manifest["phase"],"runs":6,"rows":len(table),"holdout_accessed":False,"result_dir":str(result_root)},indent=2)); return 0
