"""Assemble a D1 round according to its explicit lineage, excluding virtual links."""
import json,sys
from pathlib import Path
import FreeCAD as App,FreeCADGui as Gui,Import,Mesh,Part
job=json.loads(Path(sys.argv[-1]).read_text());root=Path(job['root']);here=root/'experiments/try5A';T=here/'try5A_2';r=int(job['round']);line=json.loads((T/f'D1/round_{r}/lineage.json').read_text());s=json.loads((here/'blackboard/kinematic_skeleton.json').read_text());out=T/f'D1/round_{r}/assembly';Gui.showMainWindow();doc=App.newDocument(f'D1Round{r}');objs=[]
try:
 for link in s['links']:
  lid=link['link_id']
  ...
  for a in range(3):
   for b in range(3):setattr(m,f'A{a+1}{b+1}',M[a][b])
 ...
 (out/'assembly_result.json').write_text(json.dumps({'status':'SUCCESS','round':r,'physical_links':len(objs)},indent=2)+'\n')
