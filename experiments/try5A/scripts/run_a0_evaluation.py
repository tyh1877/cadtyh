...
meshes={x:trimesh.load(HERE/f'A0/{x}/model.stl',force='mesh',process=False) for x in links}
def bounds_at(lid,T):
...
def ref(link,jid):return next(x for x in json.loads((HERE/f'A0/{link}/InterfaceRefs.json').read_text())['interfaces'] if x['joint_id']==jid)
...
(HERE/'results/a0_interface_metrics.json').write_text(json.dumps({'summary':summary,'joints':rows,'nonadjacent_aabb_overlaps':nonadj},indent=2)+'\n')
samples=[]
...
(HERE/'results/a0_joint_sweep.json').write_text(json.dumps(sweep,indent=2)+'\n')
