...
from common import RESULTS, RUNS, VERSIONS, dump_json, ensure_dirs, frozen_rows, input_ir_path, write_csv  # type: ignore  # noqa: E402
...
def run_one(version: str, row: dict[str, str]) -> dict[str, Any]:
    ir_path = input_ir_path(version, row["case_id"], row["link_id"])
    out_dir = RUNS / "replay" / version / row["case_id"] / row["link_id"] / "freecad"
    ...
    if not ir_path.exists():
        return {
            ...
            "failure_reason": "input executable_cad_ir_v1_2.json missing",
    ...
    if failed:
        last = failed[-1]
        dump_json(out_dir.parent / "preflight.json", last.get("preflight"))
        dump_json(out_dir.parent / "failure_context.json", last.get("failure_context"))
...
def main() -> None:
    ...
    write_csv(RESULTS / "replay_metrics.csv", rows)
    summary = {"versions": {}, "silent_fallback_total": sum(int(r["fallback_count"]) for r in rows)}
    for version in VERSIONS:
    ...
    dump_json(RESULTS / "replay_summary.json", summary)
