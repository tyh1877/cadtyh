"""Freeze B1 evaluation and gate code before reading B1 metrics."""
import hashlib,json
...
out={'frozen_at':datetime.now(timezone.utc).isoformat(),'before_b1_metrics':True,'files':{p:sha((HERE/p).resolve()) for p in files}};(HERE/'results/evaluation_snapshot.json').write_text(json.dumps(out,indent=2)+'\n');print(json.dumps(out,indent=2))
