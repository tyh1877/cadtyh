...
| B/D use real Fusion API rather than primitive compiler | native F3D/STEP/STL plus Fusion execution log | PENDING: formal matrix must be regenerated after placement fix |
| Fusion components assembled at canonical FK pose | deterministic URDF/blueprint FK placement plus occurrence/component audit | PENDING: formal B/D rerun required; single-case smoke passed |
...
| Native editability | component/feature/joint/rebuild/native-save audit | PENDING: formal B/D rerun required; smoke passed |
| All B/D geometry sent to evaluator comes from Fusion occurrence STL | per-component Fusion export manifest | PENDING: formal B/D rerun required |
...
| A/B/C/D effects and paired deltas | aggregate, effect, and interaction files | PENDING: regenerate after formal B/D rerun |
| Resource and Fusion operation accounting | calls/tokens/latency/Fusion operations | PASS |
| Final report answers all applicable items with amendment caveat | audited report | PENDING: regenerate after formal B/D rerun |
